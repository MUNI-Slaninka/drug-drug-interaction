...
def cross_validation(drug_drug_matrix, CV_num, seed):
    ...
    for CV in range(0, CV_num):
        ...
        [multiple_predict_matrix, multiple_predict_results] = ensemble_method(copy.deepcopy(drug_drug_matrix),
                                                                              train_drug_drug_matrix, testPosition)
        # logstic weight
...
class Topology:
    def topology_similarity_matrix(drug_drug_matrix):
        ...
        eigenValues, eigenVectors = LA.eig(drug_drug_matrix)
        idx = eigenValues.argsort()[::-1]
        eigenValues = eigenValues[idx[0]]

        beta = 0.5 * (1 / eigenValues)
        Katz_similarity_matrix = pinv(np.identity(drug_num) - beta * drug_drug_matrix) - np.identity(drug_num)
...
class MethodHub():
    ...
    def generate_distrub_matrix(drug_drug_matrix):
        ...
        index = numpy.arange(0, (upper_A.sum()).sum())
        # print(index.shape)
    ...
    def disturb_matrix_method(train_drug_drug_matrix):
        ...
        for i in range(0, num_eigenvalues):
            reconstructed_A = reconstructed_A + (eigenvalues[i] + delta_eigenvalues[i]) * eigenvectors[:,
                                                                                          i] * np.transpose(
                eigenvectors[:, i])
...
def internal_determine_parameter(drug_drug_matrix):
    ...
    [multiple_matrix, multiple_result] = ensemble_method(copy.deepcopy(drug_drug_matrix), train_drug_drug_matrix,
                                                         testPosition)
